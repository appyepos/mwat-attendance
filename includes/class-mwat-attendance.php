...
final class MWAT_Attendance {
    ...
    private function analytics_screen() {
        ...
        foreach($filtered as $e){$type=$e['type']??'attendance';$wid=(int)($e['walk_id']??0);$week=wp_date('Y-m-d',strtotime(($e['date']??$today).' monday this week'));if(!isset($weekly[$week]))$weekly[$week]=array('att'=>0,'responses'=>0);
            if('cancelled'===$type){$cancel++;$r=$this->cancellation_label($e);$reasons[$r]=($reasons[$r]??0)+1;}else{$submitted++;$a=(int)($e['attendees']??0);$att+=$a;$new+=(int)($e['new_attendees']??0);$dogs+=(int)($e['dogs']??0);$by_walk[$wid]=($by_walk[$wid]??0)+$a;$weekly[$week]['att']+=$a;}$weekly[$week]['responses']++;}
        ...
        echo '<div class="mwat-analytics-grid"><div class="mwat-card"><div class="mwat-card-head"><div><h3>Attendance trend</h3><p>Weekly total attendance</p></div></div><div class="mwat-bars">';
        $max=1;foreach($weekly as $w)$max=max($max,$w['att']);foreach($weekly as $week=>$w){$pct=round(($w['att']/$max)*100);echo '<div class="mwat-bar-row"><span>'.esc_html(date_i18n('j M',strtotime($week))).'</span><div><i style="width:'.$pct.'%"></i></div><strong>'.$w['att'].'</strong></div>';}if(!$weekly)echo '<div class="mwat-empty">No data in this period.</div>';echo '</div></div>';
