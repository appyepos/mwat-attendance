...
final class MWAT_Attendance {
    ...
    private function analytics_screen() {
        ...
        echo '<div class="mwat-card mwat-groups-growth"><div class="mwat-card-head"><div><h3>Group growth</h3><p>New walk groups added to the website, using each GeoDirectory walk\'s published date.</p></div></div><div class="mwat-bars">';foreach($group_months as $month=>$added){$running+=$added;echo '<div class="mwat-group-growth-row"><span>'.esc_html(date_i18n('M Y',strtotime($month))).'</span><div><small>+'.(int)$added.' new</small><strong>'.$running.' groups</strong></div></div>';}if(!$group_months)echo '<div class="mwat-empty">No group dates available.</div>';echo '</div></div>';
